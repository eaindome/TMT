<script lang="ts">
	...
	const variantClasses: Record<Variant, string> = {
	  ...
	  outline: 'bg-transparent border-2 border-[#1a5f4a] text-[#1a5f4a] hover:bg-[#1a5f4a] hover:bg-opacity-5 active:bg-opacity-10',
	  ghost: 'bg-transparent text-[#1a5f4a] hover:bg-[#1a5f4a] hover:bg-opacity-5 active:bg-opacity-10'
	};
	...
	$: classes = `
	  ...
	  ${shapeClasses[shape]}
	  ${fullWidth ? 'w-full' : ''}
	  ${elevation && !disabled ? 'shadow-md hover:shadow-lg active:shadow-sm' : ''}
	  ...
	  ${loading ? 'relative' : ''}
	  ${pulse ? 'animate-pulse' : ''}
	  ${animateOnHover && !disabled ? 'transform hover:scale-[1.02] active:scale-[0.98]' : ''}
  ...
  >
	...
	{:else}
	  {#if icon && iconPosition === 'left'}
		<span class={`h-5 w-5 ${isHovered && animateOnHover ? 'transform -translate-x-0.5 transition-transform' : ''}`}>
		  {#if typeof icon === 'function'}
			<svelte:component this={icon} />
		  {/if}
		</span>
	  {/if}
	  <span class={`${isHovered && animateOnHover ? 'transform scale-105 transition-transform' : ''} ${isPressed ? 'transform scale-95' : ''}`}>
		<slot />
	  </span>
