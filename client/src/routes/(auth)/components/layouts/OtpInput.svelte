...
<div class="mb-6">
	...
	<div
		...
		class="rounded-lg border border-gray-100 bg-gray-50 p-4 transition-all duration-300"
	...
	>
			...
			{#each otp as digit, i}
				<div class="relative">
					<input
						...
						class="h-16 w-14 rounded-lg border-2 text-center text-2xl font-bold shadow-sm transition-all duration-200 focus:outline-none {digit
							? error
								? 'border-red-400 bg-red-50'
								: 'border-green-400 bg-green-50'
							: error
								? 'border-red-300 bg-white'
								: 'border-gray-300 bg-white'} focus:scale-105 focus:border-[#1a5f4a] focus:ring-2 focus:ring-[#1a5f4a]"
					/>

					<!-- Animated underline indicators -->
					<div class="mt-1 flex justify-between px-1">
						<div
							class="h-1 w-1 rounded-full {digit
								? error
									? 'bg-red-400'
									: 'bg-green-400'
								: 'bg-gray-300'}"
		...
		<div class="mt-4 flex items-center justify-between">
			...
			<button
				type="button"
				on:click={reset}
				class="text-xs text-[#1a5f4a] hover:text-[#144639] focus:text-[#144639] focus:outline-none {isComplete ||
				otp.some((d) => d !== '')
					? ''
	...
	{#if resendAction}
		<div class="mt-4 text-center">
			<button
				...
				class="text-sm font-medium transition-colors duration-300 {remainingTime > 0
					? 'cursor-not-allowed text-gray-400'
					: 'text-[#1a5f4a] hover:text-[#144639]'}"
			>
				{#if remainingTime > 0}
