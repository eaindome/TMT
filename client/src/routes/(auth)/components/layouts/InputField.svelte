...
<div class="relative mb-4">
	...
	<label
		for={id}
		class="absolute z-10 text-sm font-medium transition-all duration-200 {focused || hasValue
			? '-top-2 left-2 rounded bg-white px-1 text-xs'
			: 'top-2 left-3'} {error
			? 'text-red-600'
			: focused
				? 'text-[#1a5f4a]'
	...
	<div class="relative">
		...
		<input
			...
			on:blur={handleBlur}
			class="block w-full appearance-none rounded-md border px-3 py-2 placeholder-gray-400 shadow-sm transition-all duration-200 ease-in-out {focused
				? 'border-[#1a5f4a] ring-1 ring-[#1a5f4a]'
				: error
					? 'border-red-500 ring-1 ring-red-500'
					: validState
						? 'border-green-500 ring-1 ring-green-500'
						: 'border-[#E2E8F0] hover:border-gray-400'} {icon && iconPosition === 'left'
				? 'pl-10'
				: ''} {type === 'password' || (hasValue && !disabled && !readonly)
				? 'pr-10'
				: ''} sm:text-sm {disabled ? 'cursor-not-allowed bg-gray-100 text-gray-500' : ''}"
		/>
		...
		{#if type === 'password'}
			<button
				...
				class="absolute top-1/2 right-3 -translate-y-1/2 text-gray-400 hover:text-[#1a5f4a] focus:text-[#1a5f4a] focus:outline-none"
	...
	<div class="mt-1 flex items-start justify-between">
		...
		{#if showCharCount && maxLength}
			<p class="ml-2 text-xs text-gray-500 tabular-nums">
				{charCount}/{maxLength}
			</p>
		{/if}
	</div>
